...
[root@linux-server ~]# useradd -G hr hr01   //创建hr01用户添加到hr属组里
[root@linux-server ~]# useradd -G hr hr02   //创建hr02用户添加到hr属组里
...
对目录有w，对文件没有任何权限      $:文件在目录内，文件的删除权限是目录所拥有。 
...
```shell
[root@linux-server ~]# mkdir /opt/dir1  #创建目录             sgid（组继承）
[root@linux-server ~]# groupadd hr  #创建一个组
[root@linux-server ~]# chmod 775 /opt/dir1/  #设置权限  rwx rwx r-x
...
[root@linux-server home]# chmod 757 dir2/     rwx r-x rwx
...
[root@linux-server ~]# chattr +A file3  #固定访问时间
[root@linux-server ~]# lsattr file1 file2 file3
-----a---------- file1
----i----------- file2
-------A-------- file3
...
出现问题：有些时候，我们登录Linux系统时，会出现命令行开头为：
...
```javascript
[gongli@mode-6 ~]$ 
  问题原因：在该用户的家目录下没有环境变量，或者环境变量出现为题。所以在登录或者切换过程中出现问题，命令行开头不正常
```

![img](assets/权限管理/20151127114311028.png)


![img](assets/权限管理/20151127114401306.png)
...
[root@mode-6 ~]# 
[root@mode-6 ~]# 
[root@mode-6 ~]# cp -a /etc/skel/.bash* /home/gongli/    # 将环境变量拷贝到对应的用户家目录下
...
-rw-r--r--. 1 root   root    176 Jul 18  2013 .bash_profile
-rw-r--r--. 1 root   root    124 Jul 18  2013 .bashrc
[root@mode-6 ~]# chown -R gongli.gongli /home/gongli/.ba*    # 修改权限
[root@mode-6 ~]# ll -a /home/gongli/                     
total 20
drwxr-xr-x  2 gongli gongli 4096 Nov 27 11:33 .
drwxr-xr-x. 5 root   root   4096 Nov 27 11:26 ..
