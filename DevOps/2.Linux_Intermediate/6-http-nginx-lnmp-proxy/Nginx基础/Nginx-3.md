...
`Nginx` 有一个非常灵活的日志记录模式，每个级别的配置可以有各自独立的访问日志, 所需日志模块 `ngx_http_log_module` 的支持，日志格式通过 `log_format` 关键字来定义，日志对于统计和排错是非常有利的，下面总结了 `nginx` 日志相关的配置 包括 `access_log`、`rewrite_log`、`error_log`。
...
![img](assets/Nginx-3/1567754418355.png)



![img](assets/Nginx-3/1567754472338.png)
...
使用`log_format`指令定义了一个`main`的格式，并在`access_log`指令中引用了它。假如客户端有发起请求：`http://asuka.com/`，我们看一下我截取的一个请求的日志记录:
...
rewrite_log属于notice级别。那么记得将error_log的级别设置为notice；
...
2.上传新版本的源码包nginx-1.20.2.tar.gz，解压到/usr/local/
按照原来的编译参数安装 nginx 的方法进行安装，**只需要到 make，千万不要 make install** 。如果make install 会将原来的配置文件覆盖 

```shell
[root@nginx-server2 ~]# cd /usr/local/nginx-1.20.2/
[root@nginx-server2 nginx-1.20.2]# ./configure --prefix=/usr/local/nginx --group=nginx --user=nginx --sbin-path=/usr/local/nginx/sbin/nginx --conf-path=/etc/nginx/nginx.conf --error-log-path=/var/log/nginx/error.log --http-log-path=/var/log/nginx/access.log --http-client-body-temp-path=/tmp/nginx/client_body --http-proxy-temp-path=/tmp/nginx/proxy --http-fastcgi-temp-path=/tmp/nginx/fastcgi --pid-path=/var/run/nginx.pid --lock-path=/var/lock/nginx --with-http_stub_status_module --with-http_ssl_module --with-http_gzip_static_module --with-pcre --with-http_realip_module --with-stream
[root@nginx-server2 nginx-1.20.2]# make
千万不要make install
```
...
```shell
[root@nginx-server2 nginx-1.20.2]# mv /usr/local/nginx/sbin/nginx /usr/local/nginx/sbin/nginx_$(date +%F)
```

4、复制新的nginx二进制文件，进入新的nginx源码包 

```shell
[root@nginx-server2 nginx-1.20.2]# cp /usr/local/nginx-1.20.1/objs/nginx /usr/local/nginx/sbin/
```

5、测试新版本的nginx是否正常 

```shell
[root@nginx-server2 nginx-1.20.2]# /usr/local/nginx/sbin/nginx -t
```
...
```shell
[root@nginx-server2 nginx-1.20.2]# kill -USR2 `cat /var/run/nginx.pid`
```

7、查看nginx pid，会出现一个nginx.pid.oldbin 

```shell
[root@nginx-server2 nginx-1.20.2]# ll /var/run/nginx.pid*
...
```shell
[root@nginx-server2 nginx-1.20.2]# kill -WINCH `cat /var/run/nginx.pid.oldbin`
```
...
```shell
[root@nginx-server2 nginx-1.20.2]# kill -HUP `cat /var/run/nginx.pid.oldbin`
```
...
```shell
[root@nginx-server2 nginx-1.20.2]# kill -QUIT `cat /var/run/nginx.pid.oldbin`
```

11、验证Nginx是否升级成功 

```shell
[root@nginx-server2 nginx-1.20.2]# /usr/local/nginx/sbin/nginx -V
```

12、访问验证 

```shell
[root@localhost nginx-1.20.2]# elinks 10.0.105.189
```
...
# ![img](assets/Nginx-3/1561964133748.png)Nginx 流量限制
...
`burst`参数定义了超出zone指定速率的情况下(示例中的`mylimit`区域，速率限制在每秒1个请求，或每1000毫秒一个请求)，客户端还能发起多少请求。上一个请求1000毫秒内到达的请求将会被放入队列，我们将队列大小设置为20。



这意味着，如果从一个给定IP地址发送21个请求，Nginx会立即将第一个请求发送到上游服务器群，然后将余下20个请求放在队列中。然后每1000毫秒转发一个排队的请求，只有当传入请求使队列中排队的请求数超过20时，Nginx才会向客户端返回503错误。

### 3.1、无延迟的排队

配置burst参数将会使通讯更流畅，但是可能会不太实用，因为该配置会使站点看起来很慢。在上面的示例中，队列中的第20个包需要等待2秒才能被转发（rate=10r/s），此时返回给客户端的响应可能不再有用。要解决这个情况，可以在burst参数后添加nodelay参数
...
limit_req zone=mylimit burst=4 nodelay; 表示处理 mylimit 的请求队列， burst=4 表示2个并发堆满了之后会把请求放请求 mylimit 队列， nodelay 表示进入请求队列的请求也会被立即执行

limit_req_status （此行可选）为超出并发队列的请求返回的自定义http状态码

burst=4和nodelay为可选参数，写nodelay时必须写burst，写burst时不必写nodelay。

不加nodelay请求队列中请求会排队等待执行，此时并发限制死为burst。

加上nodelay表示队列中请求也会被立即执行，超出burst的请求会直接被拒绝，此模式可理解为nginx允许临时突破并发量到burst值。（上述示例表示并发限制为2，可以临时突破到4，超过4的直接返回503）
...
上面配置了单个IP同时并发连接数最多只能10个连接，并且设置了整个虚拟服务器同时最大并发数最多只能100个连接。当然，只有当请求的header被服务器处理后，虚拟服务器的连接数才会计数。刚才Nginx是基于漏桶算法原理实现的，实际上限流一般都是基于漏桶算法和令牌桶算法实现的。
...
![img](assets/Nginx-3/1583466862249.png)
...
![img](assets/Nginx-3/1583466914787.png)
...
![img](assets/Nginx-3/1561996355328.png)
